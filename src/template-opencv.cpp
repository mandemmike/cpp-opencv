...
cv::Mat hsvConversion(cv::Mat img)
{
  ...
  cv::Mat thresh;
  cv::inRange(hsvImage, cv::Scalar(98, 104, 30), cv::Scalar(127, 191, 112), thresh); // finding blue cones
  return thresh;
}
...
cv::Mat findConeCenter(cv::Mat img, double originalSteering)
{
  ...
  //{
  	...
  	double numerator = (mc[1].y*(150 - mc[0].x)) + (365 * (mc[0].x - mc[1].x)) + (mc[0].y * (mc[1].y - 150));
  	double denominator = (mc[1].x - 150) * (150 - mc[0].x) + (mc[1].y - 365 - mc[0].x);
 	...
 	if (steeringAngle > 1 || steeringAngle < (-1)){
     	steeringAngle = steeringAngle / 100;
 	} /*else {
  	...
  	if (sharedMemory && sharedMemory->valid())
  	{
      	...
      	while (od4.isRunning())
      	{
          	...
          	// If you want to access the latest received ground steering, don't forget to lock the mutex:
          	{
              	...
              	std::cout << "main: groundSteering = " << originalSteering << std::endl;
          	}
          	// Display image on your screen.
          	if (VERBOSE)
          	{
              	cv::Mat resultingImg = findConeCenter(img, originalSteering);
